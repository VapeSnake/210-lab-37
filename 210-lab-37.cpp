...
int main()
{
    ...
    do
    {
        ...
        {
        case 1:
        { // Display the contents of the hash map, limited to the first 100 entries for readability.
            ...
            for (const auto &pair : hashMap)
            {
                ...
                count++;
                if (count >= 100)
                    break; // Limit to first 100 entries.
            }
            break;
        };
        // Test values to find: D42DFA5FAE21 , 98FFABE6124B , DCA8C97E24B2
        case 2:
        { // Searches for a string in the hash table by generating its hash index and checking the corresponding list for a match.
            string searchStr;
            ...
            {
                if (str == searchStr)
                { // Check if the string matches the search string.
        ...
        case 3:
        { // Adds a new string to the hash table by generating its hash index and appending it to the list at that index.
            string newStr;
        ...
        // Test by adding 'dog123456789', and 'chicken12345'. Delete both. Attempt to delete cat123456789 which does not exist.
        case 4:
        { // Deletes string from hash table by getting its index and removing it from list at index if it exists.
            string delStr;
            cout << "Enter string to delete: ";
            cin >> delStr;

            int hashIndex = gen_hash_index(delStr);                 // Generate the hash index for the string to delete.
            auto &strList = hashMap[hashIndex];                     // Get reference to the list at the hash index.
            auto it = find(strList.begin(), strList.end(), delStr); // Find the string in the list.
            if (it != strList.end())
            { // If the string is found, erase it from the list.
                strList.erase(it);
                cout << "String " << delStr << " deleted from index " << hashIndex << endl;
            }
            else
            {
                cout << "String not found. Cannot delete." << endl;
            }
            break;
        }
        case 5:
